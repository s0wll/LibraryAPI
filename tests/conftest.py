...
from src.schemas.users import UserAddRequest, UserAddToDB
...
async def setup_database(check_test_mode) -> None:
    ...
    users_data[0].hashed_password = AuthService().hash_password(
        password=users_data[0].hashed_password
    )
...
async def authentificated_admin_ac(ac: AsyncClient) -> AsyncGenerator[AsyncClient]:
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        response = await ac.post(
            "/auth/login", json={"email": "admin@example.com", "password": "admin_password"}
        )
...
async def register_user(ac, setup_database):
    response = await ac.post(
        "/auth/register", json={"email": "mark@gmail.com", "username": "mark", "password": "12345"}
    )
    assert response.status_code == 200
...
async def authentificated_user_ac(ac, register_user):
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        response = await ac.post(
            "/auth/login", json={"email": "mark@gmail.com", "password": "12345"}
        )
        assert response.status_code == 200
        assert ac.cookies["access_token"]
        yield ac
